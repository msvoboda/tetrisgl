...
GLvoid GLFont::BuildFont(int  size)								// Build Our Font Display List
{
	int     loop;
	...
	for (loop=0; loop<(fxCount * fyCount); loop++)						// Loop Through All Lists
	{
		...
		glNewList(fBase + loop,GL_COMPILE);				// Start Building A List
			glBegin(GL_QUADS);							// Use A Quad For Each Character
				...
				glVertex2i(size*fWidth - 1,0);						// Vertex Coord (Bottom Right)
				glTexCoord2f(cx+cwx,1-cy);			// Texture Coord (Top Right)
				glVertex2i(size*fWidth - 1,size*fHeight -1);						// Vertex Coord (Top Right)
				glTexCoord2f(cx,1-cy);					// Texture Coord (Top Left)
				glVertex2i(0,size*fHeight -1);						// Vertex Coord (Top Left)
			glEnd();									// Done Building Our Quad (Character)
			glTranslated(size*fSpacing,0,0);						// Move To The Right Of The Character
